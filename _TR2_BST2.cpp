...
class info {
public:
	int size, ans, max, min;
	bool isBST;
};

void restoreBSTutil(node*root2 , node**prev, node**mid, node**first, node**last) {
	if (root2 == NULL)return;

	restoreBSTutil(root2->left, prev , mid, first, last);

	if (*prev && root2->data < (*prev)->data) {
		if (!*first) {
			*first = *prev;
			*mid = root2;
		} else {
	...
	*prev = root2;
	restoreBSTutil(root2->right , prev, mid, first, last);
...
void restoreBST(node*root2) {
	...
	restoreBSTutil(root2, &prev, &mid, &first, &last);
...
info LargestBST(node*root) {
	...
	if (leftSubtree.isBST && rightSubtree.isBST && root->data > leftSubtree.max && root->data < rightSubtree.min ) {
		currNode.isBST = 1;
		currNode.ans = currNode.size;
		currNode.min = min(root->data , min(rightSubtree.min , leftSubtree.min) );
		currNode.max = max(root->data , max(rightSubtree.max , leftSubtree.max) );

		return currNode;
	}

	currNode.ans = max(leftSubtree.ans , rightSubtree.ans);
	currNode.isBST = 0;
...
int main(int argc, char const * argv[]) {
	...
	*/
	...
	root->left->right->left = new node(-501);

	root->right->left = new node(70);
	root->right->right = new node(80);


	root->right->left->left = new node(701);
	root->right->left->right = new node(704);
	root->right->right->left = new node(801);
	root->right->right->right = new node(804);
	...
	node *root7 = new node(11) , *root8 = new node(11);

	root7->left = new node(22) ;
	root7->right = new node(33) ;

	root7->left ->left = new node(44) ;
	root7->left ->right = new node(55) ;

	root8->left = new node(22) ;
	root8->right = new node(33) ;

	root8->left ->left = new node(44) ;
	root8->left ->right = new node(155) ;

	if (isIdentical(root7, root8))
		cout << endl << "same trees " << endl;
	else
		cout << endl << "different trees" << endl;

	cout << "Largest BST in root is : " << LargestBST(root5).ans << endl;

	node* root9 = new node(6);
	root9->left = new node(9);
	root9->right = new node(3);

	root9-> left->left = new node(1);
	root9-> left->right = new node(4);

	// root9-> right->left = new node(1);
	root9-> right->right = new node(14);
