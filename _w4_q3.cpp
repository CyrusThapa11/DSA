...
void checkPallindrome(string &s, int i) {
   ...
   if (s[i] != s[i + 1]) {
      ...
      for (int j = 0; j < s.size(); ++j) {
         if ((i - j - 1 >= 0) && (i + j + 1 < s.size()) && s[i - j - 1] == s[i + j + 1] ) {
            len++;
         } else {
            ans = 0;
            break;
      ...
      string pallind = s.substr(prevIdx, len * 2 - 1);
      for (int j = prevIdx; j <= lastIdx; ++j) {
         s[j] = '-';
      }
...
void removePallindromes(string &str) {
